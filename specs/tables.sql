...
-- ============================================================================
-- Product Maintenance Tasks Cache (Migration 054)
...
-- ============================================================================
-- Async AI Jobs (Migration 053)
-- Background job execution via Supabase Edge Function; progress via Realtime.
-- ============================================================================
...
-- ============================================================================
-- TABLE: conversations  (Migration 061)
-- One conversation per approved registration.
-- participant_1_id = crew, participant_2_id = skipper/owner.
-- ============================================================================
...
-- ============================================================================
-- TABLE: conversation_messages  (Migration 061)
-- Messages within a conversation. read_by tracks who has seen each message.
-- ============================================================================
...
create or replace function public.open_conversation_for_registration(
  p_registration_id uuid, p_crew_id uuid, p_skipper_id uuid
)
returns uuid language plpgsql security definer set search_path = public as $$
declare v_id uuid;
begin
  select id into v_id from public.conversations where registration_id = p_registration_id;
  if v_id is not null then return v_id; end if;
  insert into public.conversations (registration_id, participant_1_id, participant_2_id)
  values (p_registration_id, p_crew_id, p_skipper_id) returning id into v_id;
  return v_id;
end; $$;
...
create or replace function public.mark_messages_read(p_message_ids uuid[], p_user_id uuid)
