-- Migration: Fix viewport RPC spatial filtering - use ST_Intersects instead of ST_Within
-- Issue: Migration 043 used ST_Within which is TOO STRICT - it requires waypoint to be completely
--        inside the viewport envelope. But waypoints at the exact edge or slightly outside get filtered,
--        causing the original disappearing problem again.
-- Solution: Replace ST_Within with ST_Intersects which checks if waypoint geometry overlaps viewport.
--          This is consistent with how bbox filtering already works (loose check for performance),
--          but applied to actual waypoints instead of huge bboxes.
...
-- Create the function with ST_Intersects instead of ST_Within
...
BEGIN
  ...
  WHERE
    ...
    -- CRITICAL FIX: Require that legs have a start waypoint (essential for displaying on map)
    -- This filters out incomplete legs that would otherwise be fetched but not rendered
    AND EXISTS (
      SELECT 1 FROM public.waypoints w
      WHERE w.leg_id = l.id
        AND w.index = 0
    )
    -- IMPORTANT: Trust the bbox filter AND the client-side rendering
    -- The bbox filter (&&) already ensures legs intersect the viewport
    -- The client-side rendering with fallback logic ensures waypoints display correctly
    -- Do NOT add additional strict spatial filtering here as it causes false negatives
    -- when waypoints are at viewport edges or when viewport is very small
