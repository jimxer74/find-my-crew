-- Migration: Fix get_legs_in_viewport RPC to only return legs with start waypoints
-- Issue: RPC was returning legs without waypoints, causing volatility in viewport display
-- Solution: Add WHERE clause to require start_waypoint (index = 0) exists for all returned legs
-- This ensures only displayable legs are returned from the API
...
-- Create the function with waypoint requirement
...
BEGIN
  ...
  WHERE
    ...
    -- CRITICAL: Require that legs have a start waypoint (essential for displaying on map)
