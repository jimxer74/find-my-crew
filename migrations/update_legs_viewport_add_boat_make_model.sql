-- Update get_legs_in_viewport function to return boat_make and boat_model instead of skipper_name
...
RETURNS TABLE (
  ...
  boat_make text,  -- Boat make
  boat_model text,  -- Boat model
  owner_name text,  -- Owner's full name from profiles
  owner_image_url text,  -- Owner's profile image URL
...
-- Add comment
COMMENT ON FUNCTION get_legs_in_viewport(double precision, double precision, double precision, double precision, date, date, risk_level[], text[], integer) IS 'Returns legs within a viewport for crew browsing. Uses PostGIS bbox for fast spatial queries. Only returns legs from published journeys. Supports optional filters for dates, risk levels, skills, and minimum experience level. Returns boat_make, boat_model, owner_name, and owner_image_url.';
