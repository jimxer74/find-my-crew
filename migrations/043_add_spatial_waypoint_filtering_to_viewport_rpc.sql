-- Migration: Add spatial waypoint filtering to get_legs_in_viewport RPC
-- Issue: Legs with far-apart waypoints (e.g., London to New York) had huge bboxes that intersected viewport
--        but their actual waypoints were outside the visible area. This caused waypoints to disappear when
--        they moved into the 10% edge zone and then went outside viewport bounds.
-- Solution: Add WHERE clause to require at least one waypoint is spatially WITHIN the viewport envelope
--          This ensures only legs with actually-visible waypoints are returned
...
-- Create the function with spatial waypoint filtering
...
BEGIN
  ...
  WHERE
    ...
    -- NEW CRITICAL FIX: Require at least one waypoint is spatially WITHIN the viewport
    -- This prevents legs with huge bboxes (spanning continents) from being returned when
    -- their actual waypoints are outside the visible area
    -- Note: We check both start and end waypoints to handle both departure and arrival filters
    AND EXISTS (
      SELECT 1 FROM public.waypoints w
      WHERE w.leg_id = l.id
        AND (
          w.index = 0  -- Start waypoint
          OR w.index = (  -- End waypoint (highest index)
            SELECT MAX(w2.index)
            FROM public.waypoints w2
            WHERE w2.leg_id = l.id
          )
        )
        AND ST_Within(
          w.location,
