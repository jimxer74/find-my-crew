...
export default function BoatEquipmentPage({ params }: { params: Promise<{ boatId: string }> }) {
  ...
  const handleEdit = (item: BoatEquipment) => {
    setEditingItem(item);
    setIsFormOpen(true);
  };
  ...
  const handleDelete = async (item: BoatEquipment) => {
    try {
      const res = await fetch(`/api/boats/${boatId}/equipment/${item.id}`, {
        method: 'DELETE',
      });
      if (res.ok) {
        ...
        const json = await res.json();
        logger.error('Failed to delete equipment', { error: json.error });
      }
    } catch (err) {
  ...
  // Build parent options for the form (top-level equipment items)
