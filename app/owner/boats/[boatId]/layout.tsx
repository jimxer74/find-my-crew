  ...
  return (
    <div className="min-h-screen bg-background">
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
        {/* Boat header with close button */}
        <div className="flex items-center justify-between mb-6">
          <div className="flex-1 min-w-0">
            <div className="flex items-baseline gap-2">
              <h1 className="text-xl sm:text-2xl font-bold text-foreground truncate">{boat.name}</h1>
              {boat.make_model && <span className="text-sm text-muted-foreground truncate">{boat.make_model}</span>}
            </div>
          </div>
          <button
            onClick={() => router.push('/owner/boats')}
            className="flex-shrink-0 ml-4 p-2 text-muted-foreground hover:text-foreground hover:bg-muted rounded-lg transition-colors"
            aria-label="Close"
          >
