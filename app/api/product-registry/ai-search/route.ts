...
export async function POST(request: NextRequest) {
  try {
    ...
    // Save to product_registry — upsert new, enrich existing (empty fields only)
    if (valid.length > 0) {
      ...
      const { data: saved, error: rpcError } = await supabase
        .rpc('upsert_and_enrich_product_registry', { items: rows });

      if (rpcError) {
        logger.warn('[product-ai-search] RPC upsert failed', { error: rpcError.message });
