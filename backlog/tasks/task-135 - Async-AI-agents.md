...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 A shared `async-jobs` module exists under `shared/lib/async-jobs/` with a typed API for submitting, tracking, and consuming async background AI jobs
- [ ] #2 Existing synchronous AI flows continue to work unchanged — short interactions use the current request-response pattern with no async overhead
- [ ] #3 The shared module exposes a consistent TypeScript interface such that switching a workflow from sync to async requires minimal consumer-side code change
- [ ] #4 A background worker (Supabase Edge Function) can execute the full AI tool loop without hitting any serverless timeout (job runs to completion regardless of duration)
- [ ] #5 Each async tool-loop iteration emits a progress event that includes: current step label, optional percentage, and any intermediate AI message content
- [ ] #6 Client can subscribe to real-time progress updates for a given async job ID via Supabase Realtime and display them while the job is running
- [ ] #7 Client receives a clear completion event (success or failure) for async jobs and can retrieve the final result
- [ ] #8 A workflow can be triggered asynchronously by user action or by a scheduler (pg_cron)
- [ ] #9 At least one slow AI workflow (owner onboarding journey generation) is migrated to async mode as proof-of-concept, while simpler steps in the same flow remain on the sync path
- [ ] #10 Intermediate AI messages from the async tool loop are preserved and surfaced to the client (resolves doc-012 issue)
- [ ] #11 Async jobs are scoped to the owning user via Supabase RLS — users cannot access other users' jobs or progress
<!-- AC:END -->
...
- [ ] #11 Dual-mode design is documented: which existing workflows use sync, which are migrated to async, and the criteria for choosing between them
<!-- DOD:END -->
