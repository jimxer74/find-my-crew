---
id: TASK-055
title: Fix UI issues
status: In Progress
assignee: []
created_date: '2026-01-29 08:57'
updated_date: '2026-02-18 13:26'
labels: []
dependencies: []
---

## Description

<!-- SECTION:DESCRIPTION:BEGIN -->
- Light mode texts does not work in Chrome mobile browser
- Implement  "Waiting" indicator for long running actions. registration. fetch boat data, etc.
- Register for Leg button is not usable, it does not react to click may cause double clickings --> needs to provide beter feedback to user that is already clicked
- After Register leg the REgister form is still visible
- Prospect ai assistant, show badge buttons to verify profile
- Add "Exit assistant" functionality to owner chat as well
- /crew page when user signs-in the Filters should be preset based on the user's profile
- /crew/dashboar when user opens map view after sign-up the filters should be based on user profile setting
- /crew list page displays only one cruising area with more button to user with predefined location preference Panama
- Add headers to skills in profile helper pane
- Fix the validations in forms and notification banner so that it is visible to users
<!-- SECTION:DESCRIPTION:END -->
