---
id: TASK-039
title: >-
  Expandable / collapsible bottom sheet in mobile version to list legs visible
  in map viewport
status: To Do
assignee: []
created_date: '2026-01-28 06:42'
labels: []
dependencies: []
---

## Description

<!-- SECTION:DESCRIPTION:BEGIN -->
Similar to Airbnb mobile version, a bottom sheet that user can expand and make smaller. Bottom sheet lists the legs found in the users current viewport, organized in order of the best match. Information displayed:
- Boat images (large in top) in carousel, swiping left and right displays a next image
- Match badge on top left, shows the match percentage
- Under the boat image: Leg information, start and end places, dates, duration
- Clicking opens the leg detail view
<!-- SECTION:DESCRIPTION:END -->
